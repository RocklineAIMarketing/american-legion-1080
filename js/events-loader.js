(function () {
  ...
  const SHEET_CSV_URL = "PASTE_PUBLISHED_EVENTS_SHEET_CSV_URL_HERE";

  const listEl = document.getElementById("events-list");
  if (!listEl) return;

  if (!SHEET_CSV_URL || SHEET_CSV_URL.indexOf("PASTE_") === 0) {
    listEl.innerHTML = '<p class="events-status">Event list not connected yet.</p>';
    return;
  }
  ...
  fetch(SHEET_CSV_URL)
    ...
    .then((csvText) => {
      ...
      if (!rows.length) {
        listEl.innerHTML = '<p class="events-status">No upcoming events posted yet — check back soon!</p>';
        return;
      }
      ...
      if (nameIdx === -1) {
        listEl.innerHTML = '<p class="events-status">Event sheet needs an "Event Name" column.</p>';
        return;
      }
    ...
    .catch((err) => {
      listEl.innerHTML = '<p class="events-status">Couldn\'t load events right now — check back soon!</p>';
